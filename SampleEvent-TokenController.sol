...
contract Event is TokenController, Owned {

    uint public startSaleTime;       // In UNIX Time Format
    uint public endSaleTime;         // In UNIX Time Format
    uint public maximumNumberOfTickets;         // In wei
...
/// @param _tokenAddress Address of the token contract this contract controls
    ...
    function Event(
        uint _startSaleTime,
        uint _endSaleTime,
        uint _maximumNumberOfTickets,
        address _vaultAddress,
        address _tokenAddress

    ) {
        require ((_endSaleTime >= now) &&           // Cannot end in the past
            (_endSaleTime > _startSaleTime) &&
            (_maximumNumberOfTickets <= 10000) &&        // The Beta is limited
            (_vaultAddress != 0));                    // To prevent burning ETH
        startSaleTime = _startSaleTime;
        endSaleTime = _endSaleTime;
        maximumNumberOfTickets = _maximumNumberOfTickets;
...
// First check that the Event is allowed to receive this donation
        require ((now >= startSaleTime) &&
            ...
            (msg.value != 0) &&
            (totalCollected + msg.value <= maximumNumberOfTickets));
